...
class _CreateTicketScreenState extends State<CreateTicketScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: BlocProvider(
        create: (context) => TicketFormCubit(ticketRepository: Injector.instance.ticketRepository),
        child: BlocConsumer<TicketFormCubit, TicketFormState>(
          listener: (context, state) {
            if (state.status == TicketFormStatus.success) {
              ScaffoldMessenger.of(context)
                  .showSnackBar(SnackBar(content: Text(context.tr('ticket_created'))));
              Navigator.pop(context);
            } else if (state.status == TicketFormStatus.error) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text(state.errorMessage ?? context.tr('export_failed'))),
              );
            }
          },
              ...
              child: ListView(
                ...
                children: [
                  CustomTextField(
                    ...
                    validator: (v) =>
                        Validators.required(v, fieldLabel: context.tr('subject'), isArabic: isArabic),
                  ),
                  ...
                  CustomDropdown<TicketPriority>(
                    ...
                    onChanged: (p) {
                      if (p != null) context.read<TicketFormCubit>().changePriority(p);
                    },
                  ),
                  const SizedBox(height: 16),
                  CustomDropdown<TicketCategory>(
                    ...
                    onChanged: (c) {
                      if (c != null) context.read<TicketFormCubit>().changeCategory(c);
                    },
                  ),
