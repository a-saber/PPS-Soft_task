...
class TicketDetailCubit extends Cubit<TicketDetailState> {
  ...
  TicketDetailCubit({required this.ticketRepository, required TicketModel initialTicket})
      : super(TicketDetailState(status: TicketDetailStatus.loading, ticket: initialTicket)) {
    loadHistory();
  }
  ...
  Future<void> loadHistory() async {
    try {
      ...
      emit(state.copyWith(status: TicketDetailStatus.error, errorMessage: e.toString()));
    }
  }
  ...
  Future<void> changeStatus(TicketStatus newStatus) async {
    ...
    emit(state.copyWith(status: TicketDetailStatus.loaded, ticket: refreshed ?? state.ticket));
    await loadHistory();
  }
