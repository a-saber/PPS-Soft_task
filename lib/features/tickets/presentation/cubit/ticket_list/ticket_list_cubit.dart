...
class TicketListCubit extends Cubit<TicketListState> {
  ...
  TicketListCubit({required this.ticketRepository}) : super(const TicketListState());

  Future<void> loadTickets() async {
    emit(state.copyWith(status: TicketListStatus.loading));
    try {
      final tickets = await ticketRepository.getAllTickets();
      emit(state.copyWith(status: TicketListStatus.loaded, allTickets: tickets));
    } catch (e) {
      emit(state.copyWith(status: TicketListStatus.error, errorMessage: e.toString()));
    }
  }
